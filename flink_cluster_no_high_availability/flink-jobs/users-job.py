...
def main():
    ...
    env.enable_checkpointing(10000)  # 10 sec
    storage = FileSystemCheckpointStorage("s3a://apache-flink/checkpoints/users-job/")
    cfg = env.get_checkpoint_config()
    cfg.set_checkpoint_storage(storage)
    ...
    # Set job name
    cfg = table_env.get_config().get_configuration()
    cfg.set_string('pipeline.name', 'ValidDetector')
